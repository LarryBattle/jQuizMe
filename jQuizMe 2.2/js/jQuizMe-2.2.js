	...
	$.fn.jQuizMe = function( wordList, options, userLang ){
		...
		return this.each( function(){
			...
			setCheckAndNextBtn = function(){
				var isFlashCard = '';
				$( ".q-check-btn", currQuiz ).click( function( e ){
					...
					if( !letUserAnswerAgain( isFlashCard ) ){
						...
						$( '.q-check-btn', currQuiz ).hide();
						$( ".q-next-btn", currQuiz ).show();
					}
					else{
...
//					$( e.target ).attr( "disabled", true );
			...
			changeProb = function( isReview ){
				...
				if( !isReview ){ 
					...
					setQResultDisplay( 0 );
					$( ".q-check-btn", currQuiz ).show();
					$( ".q-next-btn", currQuiz ).hide();
			...
			getStatsProblem = function( i ){
				...
				if( !stats.problem[ i ] ){
					stats.problem[ i ] = { 
						"amountTried": 0, "isCorrect":null, "userAnswer": undefined,
						'question': q.ques[ i ], 'answer': q.ans[ i ]
					};
				}
