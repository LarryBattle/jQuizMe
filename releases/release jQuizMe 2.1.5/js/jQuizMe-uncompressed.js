// Beta Release : jQuizMe 2.1.5 by Larry Battle.
...
// Note: Retry is defined as a number of chances that a user will get to answer a question correctly before advancing to the next.
...
(function($){	
	...
	convertStrToHTMLUnicode = function( str, allow ) {
		if( typeof str !== "string" || !allow ){ return str; }
	...
	convertArrayToHTMLUnicode = function( arr, allow ){
		if( isArray( arr ) ){
			var i = arr.length;
			while( i-- ){
				arr[ i ] = convertStrToHTMLUnicode( arr[ i ] );
			}
		}
		return arr;
	},
	isArray = $.isArray,
	isBadMSIE = $.browser.msie,
	_lang = {
	...
	$.fn.jQuizMe = function( wordList, options, userLang ){
		...
		return this.each( function(){
			...
			stats = { 
				totalQues: 0,
				quesTried: 0,
				amountTried: [], // Holder for the amount of retries on a question.
				// rightAns[] and wrongAns[] keeps an track of the q's answer index, enabling the user to review on gameOver.
				rightAns: [], 
				wrongAns : [],
				accur : function(){
					var x = Math.round( this.rightAns.length / this.quesTried * 100 );
				...
				perc : function(){ 
					var x = Math.round( this.rightAns.length / this.totalQues * 100 );
			...
			haveCorrAnsWithinRetries = function( isFlashCard ){
				return (stats.amountTried[ currIndex ]-1 <= q.retryCount[ currIndex ] && checkAns( isFlashCard ));
			},
			letUserAnswerAgain = function( isFlashCard ){
				return (settings.enableRetry && stats.amountTried[ currIndex ] < q.retryCount[ currIndex ] && !checkAns( isFlashCard ));
			},
			...
			setCheckAndNextBtn = function(){
				...
				$( ".q-check-btn", currQuiz ).attr( "disabled", settings.blockCheck )
					.click( function( e ){
						isFlashCard = $( ".userInputArea", currQuiz ).triggerHandler( "getUserAns" );
						stats.amountTried[ currIndex ] = (stats.amountTried[ currIndex ] || 0);
						if( ++stats.amountTried[ currIndex ] == 1){
							stats.quesTried++;
						}
						if( !letUserAnswerAgain( isFlashCard ) ){
							stats[ ( checkAns( isFlashCard ) || isFlashCard ) ? "rightAns" : "wrongAns" ].push( currIndex );
			...
			checkAns = function( isFlashCard ){
				...
				if( isArray( ans ) ){
					...
					isAnsCorr = ( userAns[currIndex] == ans.toString() || ($.inArray( userAns[currIndex], ans ) + 1));
				}
				else{
			...
			getUserAnsForDisplay = function( toUni ){
				...
				return ( !userAns[currIndex] ) ? "<del>" + ans +"</del>" :
							ans + convertStrToHTMLUnicode( userAns[ currIndex ], toUni );
			},
					...
					// ansSel = <select>[ <option> is the answer + <option> * settings.multiLen]
						...
						$( d ).addClass( "userInputArea" ).bind( "getUserAns", function(){		
							if( olVer ){ 
								...
								userAns[currIndex] = (isBadMSIE) ? elText.substring( 2, elText.length ) : elText;
								userAns[currIndex] = $.trim( userAns[currIndex] );
							}
							else{
			...
			setCheckButtonView = function(){
				$( ".q-check-btn", currQuiz )[ ( settings.showFeedback)?"show":"hide"]();
			...
			setToBeginningView = function(){
				...
				$( ".q-check-btn, .q-next-btn", currQuiz ).attr( "disabled", true );
			},
			...
			checkQTypeKeys = function( quesKeys ){
				var badKey = "", i = quesKeys.length,
					validKeys = { ques:1, ans:1, ansInfo:1, ansSel:1, retry:1 };
